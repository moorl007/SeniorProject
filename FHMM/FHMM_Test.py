...
train_data.set_window(start='2013-01-05', end='2013-01-10')
...
fhmm.import_model('FHMM/trained_model.h5')

ground_truth, predictions = predict(hart, test_elec, sample_period, train_data.metadata['timezone'])
...
df_mains = next(test_elec.mains().load(sample_period=60))
df_mains['power']['active'].plot(label='GT',c='darkgreen')
plt.legend()
plt.savefig('Output/fhmm_aggregate')
plt.close()
